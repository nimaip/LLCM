...
from diffusers.optimization import EMAModel
...
def train_llcm():
    ...
    try:
        ...
        ema_unet = EMAModel(llcm_unet.parameters(), decay=config.EMA_DECAY)
    ...
    for epoch in range(config.LLCM_EPOCHS):
        ...
        for step, batch in progress_bar:
            try:
                ...
                with autocast():
                    ...
                    # Reshape for cross-attention: (batch, 1, QPI_CONTEXT_DIM)
                    qpi_context = qpi_context.unsqueeze(1)
                    ...
                    student_output = llcm_unet(noisy_t, t, encoder_hidden_states=qpi_context)
                    
                    with torch.no_grad():
                        # Access EMA model correctly
                        try:
                            ema_model = ema_unet.averaged_model if hasattr(ema_unet, 'averaged_model') else ema_unet.module
                        except AttributeError:
                            ema_model = llcm_unet  # Fallback to regular model
                        teacher_output = ema_model(noisy_t_prime, t_prime, encoder_hidden_states=qpi_context)
                ...
                ema_unet.step(llcm_unet.parameters())
        ...
        if (epoch + 1) % 10 == 0 or (epoch + 1) == config.LLCM_EPOCHS:
            try:
                ema_model = ema_unet.averaged_model if hasattr(ema_unet, 'averaged_model') else ema_unet.module
                torch.save(ema_model.state_dict(), config.LLCM_EMA_PATH)
            except AttributeError:
                # Fallback: save regular model
                torch.save(llcm_unet.state_dict(), config.LLCM_EMA_PATH)
